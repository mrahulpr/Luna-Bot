...
from shared import ACTIVE_QUIZZES, quizzes_col, settings_col, progress_col, log_error
...
async def play_topic_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- NEW PROGRESS LOGIC STARTS HERE ---
    # Fetch completed questions for this specific group and topic
    progress_data = await progress_col.find_one({"chat_id": chat_id, "topic": topic})
    completed_ids = progress_data.get("completed_q_ids", []) if progress_data else []
    
    # Fetch all questions that are NOT IN the completed list
    cursor = quizzes_col.find({
        "topic": topic,
        "_id": {"$nin": completed_ids}
    })
    questions = await cursor.to_list(length=None)
    
    if not questions:
        await query.edit_message_text(f"🌟 Awesome! This group has already completed all available questions for **{topic}**.\n\nAdmins can reset this progress in the Admin Panel.")
        return
        
    # Randomize the uncompleted questions!
    random.shuffle(questions)
    # --- NEW PROGRESS LOGIC ENDS HERE ---
    ...
    await query.edit_message_text(f"🚀 *Starting {topic} Quiz!*\n\n{len(questions)} questions remaining. Get ready...", parse_mode="Markdown")
...
async def post_next_question(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if idx >= len(quiz["questions"]):
        await context.bot.send_message(chat_id, "🎉 *The quiz has ended!* All available questions are completed. Thanks for playing.", parse_mode="Markdown")
...
async def handle_quiz_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_ans == correct_ans:
        ...
        if quiz["timer_task"] is None:
            ...
            # --- NEW SAVING PROGRESS LOGIC ---
            try:
                await progress_col.update_one(
                    {"chat_id": chat_id, "topic": quiz["topic"]},
                    {"$addToSet": {"completed_q_ids": current_q["_id"]}},
                    upsert=True
                )
            except Exception as e:
                print(f"⚠️ Failed to save group progress: {e}")
            # --- END NEW LOGIC ---
